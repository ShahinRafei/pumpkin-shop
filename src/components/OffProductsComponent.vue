        ...
        <swiper :navigation="true" :slidesPerView="'auto'" :grabCursor="true" :spaceBetween="5" :freeMode="true" :modules="modules">
            ...
            <swiper-slide v-for="product in productsList" :key="product">
                <div class="w-28 sm:w-36 md:w-44 lg:w-52 bg-white p-3 shadow-md rounded-md">
                    ...
                    <div class="w-full mb-2 font-serif whitespace-nowrap text-ellipsis overflow-hidden text-xs md:text-sm lg:text-base">{{ product.title }}</div>
                    <div class="flex justify-between items-start flex-wrap sm:mb-2">
                        ...
                        <div class="bg-orange-600 rounded-full px-2 md:px-3 text-white font-mono text-xs md:text-sm lg:text-base">10%</div>
                    </div>
                </div>
            </swiper-slide>
        </swiper>
